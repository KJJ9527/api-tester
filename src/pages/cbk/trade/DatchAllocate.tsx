...
const responseParamDefinitions: ParamDefinition[] = [
  ...
  { name: 'out_trade_no', type: 'string', length: 32, required: '是', description: 'CBK分账交易订单号,分账完成后生成的唯一流水号' },
  { name: 'allocate_date', type: 'string', length: 32, required: '否', description: '分账发起日期' },
  {
    name: 'allocate_in_list', type: 'string', length: 1024, required: '否', description: '入账订单集合',
    childrenFields:
      [
        { name: 'account_in', type: 'string', length: 15, required: '是', description: '入账户CBK账号' },
        { name: 'allocate_amt', type: 'Number', length: 11, required: '是', description: '分账入账金额 单位：分' },
        { name: 'allocate_trace', type: 'string', length: 32, required: '否', description: '入账流水号' },
        { name: 'sub_trade_no', type: 'string', length: 32, required: '是', description: '利楚入账子订单号' },
        { name: 'trade_status', type: 'string', length: 2, required: '是', description: '分账状态 1：成功 2：失败 3：处理中 4.待分账' },
        { name: 'allocate_msg', type: 'string', length: 128, required: '是', description: '子订单交易状态描述' },
      ]
  },
