...
const paramDefinitions: ParamDefinition[] = [
  ...
  { name: 'pay_type', type: 'string', length: 3, required: '是', description: '支付方式:010 微信,020 支付宝,060 qq钱包,090 抖音支付,100 翼支付,110 银联云闪付' },
  { name: 'service_id', type: 'string', length: 3, required: '是', description: '接口类型，当前类型 012' },
  ...
  { name: 'sub_appid', type: 'string', length: 16, required: '是', description: '传商户自己的微信公众号appid，或支付宝生活号appid。（即获取open_id所使用的appid）。 pay_type为010及020时必填' },
  ...
  { name: 'timeout_express', type: 'string', length: 3, required: '否', description: '订单交易关闭时间（单位分，不传默认120）。 如果公众号有设置订单失效时间的，建议和订单失效时间设置一致。该字段部分收单通道支持，若遇到不生效的情况，即为收单通道不支持。' },
...
const responseParamDefinitions: ParamDefinition[] = [
  ...
  { name: 'appId', type: 'string', length: 16, required: '否', description: '微信公众号支付返回字段，公众号id' },
  { name: 'timeStamp', type: 'string', length: 32, required: '否', description: '微信公众号支付返回字段，时间戳，示例：1414561699，标准北京时间，时区为东八区，自1970年1月1日 0点0分0秒以来的秒数。注意：部分系统取到的值为毫秒级，需要转换成秒(10位数字)' },
  { name: 'nonceStr', type: 'string', length: 32, required: '否', description: '微信公众号支付返回字段，随机字符串' },
  { name: 'package_str', type: 'string', length: 128, required: '否', description: '微信公众号支付返回字段，订单详情扩展字符串，示例：prepay_id=123456789，统一下单接口返回的prepay_id参数值，提交格式如：prepay_id=' },
  { name: 'signType', type: 'string', length: 32, required: '否', description: '微信公众号支付返回字段，签名方式，示例：MD5,RSA' },
  { name: 'paySign', type: 'string', length: 32, required: '否', description: '微信公众号支付返回字段，签名' },
  { name: 'ali_trade_no', type: 'string', length: 32, required: '否', description: '支付宝JSAPI支付返回字段用于调起支付宝JSAPI' },
  { name: 'redirect_uri', type: 'string', length: 512, required: '否', description: '云闪付支付或一些银行通道会返回。若该字段有值，请取该字段的链接做重定向跳转，不需要按照正常jsapi方式调用' },
  { name: 'token_id', type: 'string', length: 32, required: '否', description: 'qq钱包公众号支付' },
...
const Jspay: React.FC = () => {
  const defaultRequestJson = useMemo(() => getDynamicDefaultRequestJson(), []);
  return (
    <ApiTester
      method="POST"
      path="/pay/open/jspay"
      description="J公众号下单接口"
