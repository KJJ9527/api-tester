...
const paramDefinitions: ParamDefinition[] = [
  ...
  { name: 'account_cardno', type: 'string', length: 4, required: '否', description: '结算卡号' },
...
const responseParamDefinitions: ParamDefinition[] = [
  ...
  {
    name: 'card_info', type: 'string', length: 32, required: '是', description: '银行卡列表 JSON字符串格式',
    // 子表格的数据
    childrenFields: [
      { name: 'account_cardno', type: 'string', length: 32, required: '是', description: '结算卡号' },
      { name: 'account_name', type: 'string', length: 64, required: '是', description: '结算户名' },
      { name: 'bank_name', type: 'string', length: 32, required: '是', description: '支行名' },
      { name: 'bank_code', type: 'string', length: 32, required: '否', description: '银联号' },
      { name: 'account_phone', type: 'string', length: 32, required: '是', description: '银行预留手机号' },
      { name: 'bind_status', type: 'string', length: 2, required: '是', description: '0：未绑定；1：绑定成功；2：绑定失败；3：绑定审核中' },
      { name: 'bind_desc', type: 'string', length: 64, required: '是', description: '绑定描述' },
      { name: 'is_default', type: 'string', length: 2, required: '是', description: '0非默认卡 1默认卡' },
      { name: 'settlement_type', type: 'string', length: 2, required: '是', description: '结算类型:1.法人结算' },
    ]
  },
